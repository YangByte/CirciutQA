...
class Encoder(Model):
    def __init__(self,
        ...
        self._encoder_output_dim = hid_dim
        self._decoder_output_dim = hid_dim

    @overrides
    def forward(self, src, source_mask):
        embedded = self.embedding(src, attention_mask=source_mask, return_dict=True, output_hidden_states=True)
...
class SimpleSeq2Seq(Model):
    ...
    def __init__(self,
        ...
        if self.point_ratio != 0:
            ...
            self.points_proj = nn.Linear(__C.FLAT_OUT_SIZE, 8)
    ...
    def forward(self,  # type: ignore
                image, source_nums, answer,
                answer_number, type, data_id, manual_program,
                ...
                for b in range(bs):
                    ...
                    for i in range(self._beam_search.beam_size):
                        ...
                        if res is not None and len(res) > 0:
                            # for j in range(4):
                            answer_t = res[-1]
                            if answer_number[b] is not None and math.fabs(res[-1] - answer_number[b]) < 0.001:
                                answer_t = answer_number[b]
                                used_hypo = hypo
                    ...
                    self.save_results[data_id[b]] = dict(manual_program=manual_program[b],
                                                         ...
                                                         right_answer=answer_number[b], answer=answer,
                                                         answer_state=answer_state)

                    flag = 1 if answer_t == answer_number[b] else 0
                    if type[b] == '简单':
                        self.low.append(flag)
                    elif type[b] == '中等':
                        self.mid.append(flag)
                    else:
                        self.high.append(flag)
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        ...
        print('Num of total, low, mid, high', len(self.new_acc), len(self.low), len(self.mid), len(self.high))
        if len(self.low) != 0:
            all_metrics.update({'low_acc': sum(self.low) / len(self.low)})
        if len(self.mid) != 0:
            all_metrics.update({'mid_acc': sum(self.mid) / len(self.mid)})
        if len(self.high) != 0:
            all_metrics.update({'high_acc': sum(self.high) / len(self.high)})
